...
import { Button, StatusBadge, TextField, ToggleField } from "../components/ui";
...
export function HostnamesView({ projects, containers, toast }: { projects: Project[]; containers: ContainerInfo[]; toast: Toast }) {
  ...
  return (
    <section className="cloudflare-manager">
      ...
      {tab === "tunnels" ? <>
        <section className="panel compact-form"><h3>Create tunnel</h3><div className="cf-manager-form"><label className="field"><span>Client</span><select value={tunnelForm.clientId} onChange={(event) => setTunnelForm({ ...tunnelForm, clientId: event.target.value })}><option value="">Choose client</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></label><TextField label="Tunnel name" value={tunnelForm.name} onChange={(name) => setTunnelForm({ ...tunnelForm, name })} /><Button disabled={busy || !tunnelForm.clientId || !tunnelForm.name} icon={<Plus size={15} />} onClick={() => void act("Creating tunnel and Docker network...", async () => { await api.createCloudflareTunnel(tunnelForm); setTunnelForm({ clientId: "", name: "" }); })}>Create tunnel</Button></div></section>
        <section className="panel compact-form"><h3>Assign service or container</h3><div className="cf-manager-form"><label className="field"><span>Tunnel</span><select value={assignmentForm.tunnelId} onChange={(event) => setAssignmentForm({ ...assignmentForm, tunnelId: event.target.value })}><option value="">Choose tunnel</option>{tunnels.map((tunnel) => <option key={tunnel.id} value={tunnel.id}>{tunnel.tunnelName}</option>)}</select></label><label className="field"><span>Project service / container</span><select value={assignmentForm.target} onChange={(event) => setAssignmentForm({ ...assignmentForm, target: event.target.value })}><option value="">Choose target</option>{targetOptions.map((target) => <option key={target.value} value={target.value}>{target.label}</option>)}</select></label><Button disabled={busy || !assignmentForm.tunnelId || !assignmentForm.target} onClick={() => void act("Connecting target to tunnel network...", async () => { const [type, first, second] = assignmentForm.target.split(":"); const project = projects.find((item) => item.folderName === first); await api.createCloudflareAssignment(type === "compose" ? { tunnelId: assignmentForm.tunnelId, projectId: project?.id, composeProject: first, composeService: second } : { tunnelId: assignmentForm.tunnelId, containerName: first }); setAssignmentForm({ tunnelId: "", target: "" }); })}>Assign</Button></div></section>
      ...
      {tab === "hostnames" ? <>
        <section className="panel compact-form"><h3>Publish hostname</h3><div className="cf-manager-form hostname-form"><label className="field"><span>Tunnel</span><select value={hostnameForm.tunnelId} onChange={(event) => void selectHostnameTunnel(event.target.value)}><option value="">Choose tunnel</option>{tunnels.map((tunnel) => <option key={tunnel.id} value={tunnel.id}>{tunnel.tunnelName}</option>)}</select></label><label className="field"><span>Zone</span><select value={hostnameForm.zoneId} onChange={(event) => setHostnameForm({ ...hostnameForm, zoneId: event.target.value })}><option value="">Choose zone</option>{(selectedTunnel ? zones[selectedTunnel.clientId] ?? [] : []).map((zone) => <option key={zone.id} value={zone.id}>{zone.name}</option>)}</select></label><label className="field"><span>Assigned target</span><select value={hostnameForm.assignmentId} onChange={(event) => setHostnameForm({ ...hostnameForm, assignmentId: event.target.value })}><option value="">Choose target</option>{tunnelAssignments.map((item) => <option key={item.id} value={item.id}>{item.targetType === "compose_service" ? `${item.composeProject} / ${item.composeService}` : item.containerName}</option>)}</select></label><TextField label="Hostname" value={hostnameForm.hostname} onChange={(hostname) => setHostnameForm({ ...hostnameForm, hostname })} placeholder="app.example.com" /><label className="field"><span>Protocol</span><select value={hostnameForm.protocol} onChange={(event) => setHostnameForm({ ...hostnameForm, protocol: event.target.value as "http" | "https" })}><option value="http">HTTP</option><option value="https">HTTPS</option></select></label><TextField label="Port" value={hostnameForm.port} onChange={(port) => setHostnameForm({ ...hostnameForm, port })} />{hostnameForm.protocol === "https" ? <ToggleField label="No TLS verify" value={hostnameForm.noTlsVerify} onChange={(noTlsVerify) => setHostnameForm({ ...hostnameForm, noTlsVerify })} /> : null}<Button disabled={busy || !hostnameForm.tunnelId || !hostnameForm.assignmentId || !hostnameForm.zoneId || !hostnameForm.hostname} icon={<Plus size={15} />} onClick={() => void act("Publishing hostname and DNS...", async () => { await api.createCloudflareHostname({ ...hostnameForm, port: Number(hostnameForm.port) }); setHostnameForm({ tunnelId: "", assignmentId: "", zoneId: "", hostname: "", protocol: "http", port: "3000", noTlsVerify: false }); })}>Publish</Button></div></section>
        <section className="panel"><h3>Managed hostnames</h3><div className="cf-manager-list">{hostnames.map((route) => <div className="cf-manager-row" key={route.id}><div><a href={`https://${route.hostname}`} target="_blank" rel="noreferrer"><strong>{route.hostname}</strong></a><span>{route.serviceTarget}</span>{route.lastError ? <span className="danger-text">{route.lastError}</span> : null}</div><StatusBadge status={route.syncStatus} /><div className="actions">{route.syncStatus === "error" ? <Button variant="secondary" disabled={busy} onClick={() => void act("Retrying hostname synchronization...", async () => { await api.retryCloudflareHostname(route.id); })}>Retry</Button> : null}<Button variant="danger" disabled={busy} onClick={() => void act("Deleting hostname and DNS...", () => api.deleteCloudflareHostname(route.id))} icon={<Trash2 size={14} />}>Delete</Button></div></div>)}</div></section>
      </> : null}
    </section>
  );
}
