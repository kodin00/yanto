...
RUN apt-get update \
  && apt-get install -y --no-install-recommends git openssh-client ca-certificates curl docker.io postgresql-client python3 python3-pip ripgrep rsync \
  ...
  && case "$TARGETARCH" in \
       ...
       arm64) frp_arch="arm64"; frp_sha="24a4fc82b4c041835103419685ea124c4d6a7dbf83d0425481c5831b4ce4b3a4" ;; \
...
EXPOSE 8080
CMD ["sh", "-c", "if [ \"$YANTO_NODE_ROLE\" = \"worker\" ]; then exec node dist/server/server/worker.js; else exec node dist/server/server/index.js; fi"]
